...
interface DataTableProps<T> {
  ...
  onRowClick?: (item: T) => void; // Función para manejar click en fila
  showDetailModal?: boolean; // Mostrar modal de detalles por defecto
  detailModalTitle?: string; // Título del modal de detalles
  detailColumns?: TableColumn<T>[]; // Columnas específicas para el modal de detalles
}
...
const DataTable = <T extends Record<string, any>>({
  ...
  onPageSizeChange,
  highlightedIds = [],
  getItemId = (item: T) => item.id,  forcePageNumber,
  onRowClick,
  showDetailModal = false,
  detailModalTitle = 'Detalles',
  detailColumns,
}: DataTableProps<T>) => {
  ...
  // Forzar navegación a una página específica
  useEffect(() => {
    if (forcePageNumber !== undefined && forcePageNumber >= 1 && forcePageNumber <= totalPages) {
      setCurrentPage(forcePageNumber);
    }
  }, [forcePageNumber, totalPages]);
  ...
  const getActionButtonClass = (variant: string = 'secondary') => {
    ...
    switch (variant) {
      case 'primary':
        return `${baseClass} text-gray-900 hover:text-black hover:bg-gray-100`;
      case 'danger':
        return `${baseClass} text-gray-900 hover:text-black hover:bg-gray-100`;
      default:
        return `${baseClass} text-gray-900 hover:text-black hover:bg-gray-100`;
    ...
    <div className={`bg-white shadow-md rounded-lg overflow-hidden ${className}`}>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">            {/* Filas con datos reales */}
              ...
              return (
                <tr 
                  ...
                  className={`${isHighlighted 
                    ...
                    : 'hover:bg-gray-50'
                  } ${(onRowClick || showDetailModal) ? 'cursor-pointer' : ''}`}
                  onClick={(e) => {
                    // Solo permitir click si no se hace en el área de acciones
                    const target = e.target as HTMLElement;
                    const isActionArea = target.closest('[data-datatable-action]') || target.closest('.datatable-icon');
                    if (!isActionArea && (onRowClick || showDetailModal)) {
                      handleRowClick(item);
                    }
                  }}
                >
                  ...
                  {actions.length > 0 && (
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <div className="flex space-x-2" data-datatable-action="true">
                        {actions
                          ...
                          .map((action, actionIndex) => (
                            <button
                              key={actionIndex}
                              onClick={(e) => {
                                e.stopPropagation(); // Prevenir el click de la fila
                                action.onClick(item);
                              }}
      ...
      {showDetailModal && (
        <DetailModal
          ...
          fields={(detailColumns || columns).map(col => ({
            key: col.key,
            label: col.header,
            render: col.render
          }))}
